...
/* Compile with -D'TEST' to enable testing of the bit array */
#ifdef TEST

#define SIZE (8 * CHAR_BIT) /* Number of bits in the bit array to be tested */

/* Function to print out the bits of a bit array prototype */
static void printBitArray(BitArray *, const size_t);

int main(void) {
    BitArray *bits = newBitArray(SIZE);
    if (bits) {
        printf("Uninitialized bit array:\n");
        printBitArray(bits, SIZE);
        setAllBits(bits);
        printf("After setting all bits:\n");
        printBitArray(bits, SIZE);
        clearBit(bits, 0);
        clearBit(bits, 7);
        clearBit(bits, 10);
        ...
        clearBit(bits, 22);
        clearBit(bits, 41);
        printf("After clearing bits 0, 7, 10, 16, 22, 41:\n");
        printBitArray(bits, SIZE);
        deleteBitArray(&bits);
        return EXIT_SUCCESS;
    } else {
        perror("Could not allocate bit array memory");
        return EXIT_FAILURE;
    }
}

/*
 * FUNCTION:    printBitArray
 * DESCRIPTION: Prints out the bits of a bit array.
 * PARAMETERS:  bits (BitArray *): The bit array to be printed.
 *              size (size_t): The number of bits stored in the bit array.
 * RETURNS:     Nothing.
 */
static void printBitArray(BitArray *bits, const size_t size) {
    size_t k;   /* Loop index */
    for (k = 0; k < size; k++) {
        if (k > 0 && k % CHAR_BIT == 0)
            putchar(' ');
        printf("%d", getBit(bits, k));
    }
    putchar('\n');
}
